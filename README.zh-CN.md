...
- 双向自动同步：拉取远程更改、提交本地修改、自动合并兼容历史并推送更新。
- 统一管理 Overleaf 与 GitHub 项目，支持平台筛选与搜索。
- 灵活的同步节奏：全局间隔、单项目独立间隔，以及随时手动同步。
- 安全优先：检测到冲突或未完成的 Git 操作时暂停自动合并并提示处理。
- 凭据管理：每个平台保存多个脱敏 Token，支持平台默认、单项目指定或系统 Git 凭据。
- 批量操作：批量启动、暂停或删除选中项目，或一键暂停／恢复全部项目。
